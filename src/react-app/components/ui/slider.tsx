    ...
    >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="border-primary ring-ring/50 size-4 rounded-4xl border bg-background shadow-sm transition-colors hover:ring-4 focus-visible:ring-4 focus-visible:outline-none block shrink-0 select-none disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  )
