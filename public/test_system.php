...
addTestResult(
    ...
    $hasBackend,
    $hasBackend ? 'Folder backend terdeteksi di ' . '.../' . basename(dirname($backendDir ?? '')) . '/' . basename($backendDir ?? '') : 'Folder backend tidak ditemukan',
...
addTestResult(
    ...
    $hasEnv,
    $hasEnv ? 'File environment .env backend tersedia' : 'File .env belum dibuat di folder backend. Silakan rename .env.example menjadi .env',
    'Path: backend/.env'
);
